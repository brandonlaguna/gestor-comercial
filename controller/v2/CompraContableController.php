<?php
use Carbon\Carbon;
class CompraContableController extends ControladorBase{
    public $conectar;
	public $adapter;
    ...
    {
        $this->frameview('v2/CompraContable/compraContable',[]);
        $this->load('v2/CompraContable/compraContableScript',[]);
        $this->load('v2/CompraContable/compraContableModals',[]);
        $this->load('v2/CompraContable/compraContableTable',[]);
    }

    public function obtenerComprasContablesAjax()
    {
        $comprobantesContables = $this->M_ComprobanteContable->obtenerComprobanteContable([
            'cc_tipo_comprobante'   => 'I'
        ]);
        $infoventaContable = '';
        $arrayVentaContable = [];
    ...
    public function nueva()
    {
        $this->Verificar->verificarPermisoAccion($this->M_Permisos->estado(2,$_SESSION["usr_uid"]));
            ...
            $this->load('v2/CompraContable/NuevaCompra/comprobanteContableScript',[]);
            $this->load('v2/CompraContable/NuevaCompra/comprobanteContableModal',[]);
            $this->frameview("v2/CompraContable/NuevaCompra/comprobanteContable",array(
    ...
    public function crearCompraContable()
    {
        ...
        $alertType  = "error";
            try {
                ...
                if(!$estadoPermiso){
                    throw new Exception("No tienes permisos para crear ésta compra, quiza haya caducado la sesion");
                }
                ...
                if(!$getArticulos){
                    ...
                    $addIngreso = $this->M_ComprobanteContable->guardarActualizarComprobanteContable([
                        'cc_id_tipo_cpte'       => 0,
                        'cc_num_cpte'           => 0,
                        'cc_cons_cpte'          => 0,
                    ...
                    foreach($listImpuestos as $dataImp){
                        $arrayImpuestos[] =[
                            'dig_registro_comprobante'  => $addIngreso,
                    ...
                    foreach($listRetenciones as $dataRe){
                        $arrayRetenciones[] =[
                            'drg_registro_comprobante'  => $addIngreso,
                ...
                if($addIngreso){
                    ...
                    if($guardarDetalleComprobante){
                        $carro->deleteCart();
                        $success    = "print/contabilidad/$addIngreso";
                        $type       = "redirect";
                        $response   = "Compra Contable creada correctamente";
                        $alertType  = "success";
                    }
                }else{
