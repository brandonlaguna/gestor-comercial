...
class ArticuloController extends Controladorbase
{
                ...
                if (isset($_POST["idarticulo"]) && !empty($_POST["idarticulo"])) {
                    ...
                    if ($updateArticulo) {
                        ...
                        echo json_encode(array(
                            "alert" => "error",
                            "title" => "Error.",
                            "message" => "No ha sido posible actualizar",
                        ));
                    }
                } else {
                    ...
                    if ($addArticulo) {
                        ...
                        echo json_encode(array(
                            "alert" => "error",
                            "title" => "Error.",
                            "message" => "No ha sido posible agregar articulo",
                        ));
                    }
                }
    ...
    public function getItem()
    {
        if (isset($_POST["data"]) && !empty($_POST["data"])) {
            ...
            foreach ($array as $search) {
                ...
                $codigos = $puc->getCodContableCompraBy($array[$i],$getCart->ci_id);
    ...
    public function getItemByBarcode()
    {
        if (isset($_POST["data"]) && !empty($_POST["data"])) {
            ...
            //en serie se hace una busqueda en los codigos contables y se trae el dato
            $i = 0;
            $articulos = $articulo->getArticuloByBarcode($code);
            $response = [];
            //si se encontro algo en articulos lo imprime
            foreach ($articulos as $articulos) {}
            if ($articulos->idarticulo != null) {
                $response = array($articulos);
            }
            $i++;
    ...
    {
        $data = (isset($_POST) && !empty($_POST["data"])) ? $_POST["data"] : false;
        ...
        switch ($data) {
            case 'comprobante':
                echo $this->frameview("articulo/loadCartComprobantes", array("items" => $items,"impuestos"=>$impuestos));
                break;

            default:
                echo $this->frameview("articulo/loadCart", array("items" => $items,"impuestos"=>$impuestos));
                break;
        }
    ...
    public function addItemToCart($data, $pos, $tercero)
    {
        if (!empty($data) && !empty($pos) && !empty($tercero)) {
            ...
            $cart = new ColaIngreso($this->adapter);
            //obtener carro padre de este usuario
            $getCart = $cart->getCart();
            foreach ($getCart as $getCart);
    ...
    public function changeCartValue()
    {
        if (isset($_SESSION["idsucursal"]) && $_SESSION["permission"] > 0) {
            if(isset($_POST['param']) && isset($_POST['value']) && isset($_POST['cdi_id'])){

                $colaingreso = new ColaIngreso($this->adapter);

                $param = cln_str($_POST['param']);
                $value = cln_str($_POST['value']);
                $cdi_id = cln_str($_POST['cdi_id']);

                $update = $colaingreso->updateValue($param,$value,$cdi_id);
