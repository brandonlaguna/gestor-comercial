<script type="text/javascript">
	...
	$(document).ready(function() {
        ...
        var btnNuevaVenta = {
			...
			action    : () => {
				window.location.href = "#compraContable/nueva";
			},
        ...
        var btnExportarExcel ={
			...
			filename  : 'Compras Contables'
		}

        configTable.buttons.push(btnNuevaVenta);
        configTable.buttons.push(btnExportarExcel);

		function ajaxArticulo() {
			$('#tblCompraContable').DataTable().ajax.reload(null, false);
		}
        var table = $('#tblCompraContable').DataTable({
				scrollY        : '70vh',
				scrollX        : true,
				scrollCollapse : true,
				fixedHeader    : true,
                ...
                ajax: function(){
                    $.ajax({
						type     : 'POST',
						url      : base_url('CompraContable?action=obtenerComprasContablesAjax'),
						data     : {},
						dataType : 'JSON',
						success  : (examenes) => {
