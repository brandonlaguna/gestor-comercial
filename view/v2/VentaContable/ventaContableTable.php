<script type="text/javascript">
	...
	$(document).ready(function() {
        ...
        var btnNuevaVenta = {
			...
			action    : () => {
				window.location.href = "#ventaContable/nueva";
			},
		...
		function ajaxArticulo() {
			$('#tblVentaContable').DataTable().ajax.reload(null, false);
		}
        var table = $('#tblVentaContable').DataTable({
				scrollY        : '70vh',
				scrollX        : true,
				scrollCollapse : true,
				fixedHeader    : true,
                ...
                ajax: function(){
                    $.ajax({
						type     : 'POST',
						url      : base_url('VentaContable?action=obtenerVentasContablesAjax'),
						data     : {},
						dataType : 'JSON',
						success  : (examenes) => {
