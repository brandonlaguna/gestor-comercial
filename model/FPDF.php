...
*******************************************************************************/
class FPDF extends EntidadBase
{
...
protected $data;
protected $letras;			   // Valor en letras
protected $resolucion;         // Resolucion y numeracion de la factura
protected $prices;
...
function __construct($orientation='P', $unit='mm', $size='A4',$adapter="")
{
	// Some checks
	$this->_dochecks();
...
function Header($data)
{
	...
	//bussiness query
		...
		INNER JOIN global g
		INNER JOIN tb_social_media sm on s.idsucursal = sm.sm_sc_id
		WHERE s.idsucursal = '".$_SESSION["idsucursal"]."' limit 1");
		if($query->num_rows > 0){
            while($row = $query->fetch_assoc()) {
				$razon_social= $row["empresa"];
				$ciudad = $row["pais"]." - ".$row["ciudad"];
				$telefono = $row["telefono"];
				$direccion = $row["direccion"];
				$email = $row["email"];
				$documento = $row["num_documento"];
				$prefijo_documento = $row["prefijo_documento"];
				$img_logo = $row["logo_img"];
            }
		}
		//more data
		$resp=[];
        ...
        $this->Cell(0,5,$ciudad,0,1,'C');
		$this->Cell(0,1,$email,0,1,'C');
...
function Footer()
{
	...
	$this->Image('media/img/logo/only-logo.jpg',0,257,40);
	$this->Image('media/img/rosources/bar.png',0,285,210);
	$angle= 90;
	$x=180;
	$y=285;
	$this->Rotate($angle,$x,$y);
    $this->Image('media/img/logo/only-slogan.png',$x,$y,100);
	$this->Rotate(0);
	$this->PriceTable();
	$this->PiePagina();
	$this->SetY(-7);
...
public function setValorEnLetras($letras)
{
	$this->letras = $letras;
}
...
function FancyTable($header, $data)
{
    ...
    $this->SetFont('','');
    // Cabecera
    $w = array(15, 70, 18, 28,28,28,40,40);
    for($i=0;$i<count($header);$i++)
	...
	foreach($data as $item){
		$cellWidth=69;//wrapped cell width
		$Width =32;
		$cellHeight=4;//normal one-line cell height
		
		//check whether the text is overflowing
		if($this->GetStringWidth($item[1]) < $cellWidth){
		...
		//write the cells
		$this->Cell(15,($line * $cellHeight),$item[0],1,0); //adapt height to number of lines		
		...
		// $xPos+=16;
		// $this->SetXY($xPos + $cellWidth, $yPos);
		// $this->Cell(40,($line * $cellHeight),$item[7],1,1); //adapt height to number of lines
		// $xPos+=25;
		// $this->SetXY($xPos + $cellWidth, $yPos);
		// $this->Cell(40,($line * $cellHeight),$item[8],1,1); //adapt height to number of lines
		// $xPos+=25;
		// $this->SetXY($xPos + $cellWidth, $yPos);
		// $this->Cell(40,($line * $cellHeight),$item[9],1,1); //adapt height to number of lines
...
function FancyTableContabilidad($header, $data)
{
	// Colores, ancho de línea y fuente en negrita
    ...
    $this->SetFont('','');
    // Cabecera
    $w = array(11, 31, 11, 11,18,18,16,26,26,26,26,30);
    for($i=0;$i<count($header);$i++)
        $this->Cell($w[$i],7,$header[$i],1,0,'L',true);
    $this->Ln();
    // Restauración de colores y fuentes
	...
	$totalLine =0;
	foreach($data as $item){
		$cellWidth=32;//wrapped cell width
		$cellHeight=3.4;//normal one-line cell height
		...
		//write the cells
		$this->Cell(10,($line * $cellHeight),$item[0],1,0); //adapt height to number of lines		
		//use MultiCell instead of Cell
		//but first, because MultiCell is always treated as line ending, we need to 
		//manually set the xy position for the next cell to be next to it.
		//remember the x and y position before writing the multicell
		...
		$xPos+=12;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(40,($line * $cellHeight),$item[3],1,1); //adapt height to number of lines
		$xPos+=10;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(40,($line * $cellHeight),$item[4],1,1); //adapt height to number of lines
		$xPos+=17;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(40,($line * $cellHeight),$item[5],1,1); //adapt height to number of lines
		$xPos+=20;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(40,($line * $cellHeight),$item[6],1,1); //adapt height to number of lines
		$xPos+=0;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(40,($line * $cellHeight),$item[7],1,1,'R'); //adapt height to number of lines
		$xPos+=26;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(40,($line * $cellHeight),$item[8],1,1,'R'); //adapt height to number of lines
		$xPos+=26;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(40,($line * $cellHeight),$item[9],1,1,'R'); //adapt height to number of lines
...
function FancyTableReporte($header, $data)
{
    // Colores, ancho de línea y fuente en negrita
    $this->SetFillColor(240,240,240);
    $this->SetTextColor(20,20,20);
    ...
    $this->SetFont('','');
    // Cabecera
    $w = array(35, 35, 35, 35,35,35,40,40);
    for($i=0;$i<count($header);$i++)
        $this->Cell($w[$i],7,$header[$i],1,0,'L',true);
    $this->Ln();
    // Restauración de colores y fuentes
	...
	$totalLine =0;
	foreach($data as $item){
		$cellWidth=35;//wrapped cell width
		...
		//write the cells
		$this->SetX(16);
		$this->Cell(35,($line * $cellHeight),$item[0],1,0); //adapt height to number of lines		
		//use MultiCell instead of Cell
		//but first, because MultiCell is always treated as line ending, we need to 
		//manually set the xy position for the next cell to be next to it.
		//remember the x and y position before writing the multicell
		...
		$xPos+=3;
		$this->SetXY($xPos + $cellWidth , $yPos);
		$this->Cell(35,($line * $cellHeight),$item[2],1,1); //adapt height to number of lines
		$xPos+=35;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(35,($line * $cellHeight),$item[3],1,1); //adapt height to number of lines
		$xPos+=35;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(35,($line * $cellHeight),$item[4],1,1); //adapt height to number of lines
		$xPos+=35;
		if($totalLine >23){
			$this->AddPage('P','A4','');
			$totalLine = 1;
...
function FancyTableReporte2($header, $data)
{
    // Colores, ancho de línea y fuente en negrita
    $this->SetFillColor(240,240,240);
    $this->SetTextColor(20,20,20);
	...
	foreach ($data as $attr) {
		$this->SetX(16);
		$this->Cell(35,(1 * 5),$attr[0],1,0); //adapt height to number of lines	
	foreach($attr[1] as $item){
		...
		//write the cells
		$this->SetX(16);
		$this->Cell(35,($line * $cellHeight)," ",1,0); //adapt height to number of lines		
		//use MultiCell instead of Cell
		//but first, because MultiCell is always treated as line ending, we need to 
		//manually set the xy position for the next cell to be next to it.
		//remember the x and y position before writing the multicell
		$xPos=$this->GetX();
		$yPos=$this->GetY();
		$this->MultiCell($cellWidth,$cellHeight,$item[0],1);
		//return the position for next cell next to the multicell
		//and offset the x with multicell width
		$xPos+=3;
		$this->SetXY($xPos + $cellWidth , $yPos);
		$this->Cell(35,($line * $cellHeight),$item[1],1,1); //adapt height to number of lines
		$xPos+=35;
		$this->SetXY($xPos + $cellWidth, $yPos);
		$this->Cell(35,($line * $cellHeight),$item[2],1,1); //adapt height to number of lines
		$xPos+=35;
...
{
	$txt = ucwords(utf8_decode($this->letras));
	$resolucion = ucfirst(utf8_decode($this->resolucion));
    // Times 12
